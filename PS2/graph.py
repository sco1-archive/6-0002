...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        # Make sure the source and destination nodes of edge are valid nodes in 
        # our graph. Separate the checks to make it easier to add more explicit
        # error messages
        if edge.get_source() not in self.nodes:
            raise ValueError
        
        if edge.get_destination() not in self.nodes:
            raise ValueError

        # Add edge to both the source node. Because this is a digraph we don't
        # need to add the edge to the destination node
        try:
            self.edges[edge.get_source()].append(edge)
        except KeyError:
            # Catch if no edges have been added yet to the source node
