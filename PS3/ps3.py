...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        # Force positive integers, assume inputs are positive
        self.width = int(width)
        self.height = int(height)

        # Generate tile array
        # Build each column and append it to a list entry, more or less building
        # a matrix. This is more intuitively (to me) accomplished with numpy but
        # this approach is straightforward enough.
        self.tiledirt = []
        for x in range(self.width):
            tmp = []
            for y in range(self.height):
                tmp.append(dirt_amount)
            self.tiledirt.append(tmp)

    def coord2tileidx(self, x, y):
        """
        Map x/y coordinates to tile indices. Room origin is assumed to be the 
        bottom left corner
        
        Returns: Tile x and y coordinates, as integers
        """
        # Floor the position float coordinate to map to the tile
        tile_x = math.floor(x)
        tile_y = math.floor(y)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        # Get tile indices from coordinates
        tile_x, tile_y = self.coord2tileidx(pos.get_x(), pos.get_y())

        # Tiles are mapped tiles[x][y]
        if not self.is_tile_cleaned(tile_x, tile_y):
            self.tiledirt[tile_x][tile_y] -= capacity

            # Set tile dirt to 0 if we go negative
            if self.tiledirt[tile_x][tile_y] < 0:
                self.tiledirt[tile_x][tile_y] = 0
    ...
    def is_tile_cleaned(self, m, n):
        ...
        """
        # Tiles are mapped tiles[x][y]
        # self.tiledirt[m][n] == 0 if the tile is clean
        return self.tiledirt[m][n] == 0

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        # Iterate through the tiles and see where the dirt equals zero
        clean_count = 0
        for x in range(self.width):
            for y in range(self.height):
                if self.tiledirt[x][y] == 0:
                    clean_count += 1
    ...
    def is_position_in_room(self, pos):
        ...
        # Test x and y coordinates to see if they're valid tiles
        # Assumes the room boundary is not in the room (e.g. ypos == height)
    ...
    def get_dirt_amount(self, m, n):
        ...
        """
        # Get tile indices from coordinates
        tile_x, tile_y = self.coord2tileidx(m, n)

        # Tiles are mapped tiles[x][y]
        return self.tiledirt[tile_x][tile_y]
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.direction = 360.0*random.random()  # Random returns 0.0 <= N < 1.0, multiply by 360 to scale
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        # Random returns 0.0 <= N < 1.0, multiply by height and width to scale
        # appripriately. Because the upper bound is not inclusive, the position 
        # is guaranteed to be inside our room
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_tile_furnished(self, m, n):
        ...
        """
        # m = x, n = y
        # Iterate through our FurnishedRoom's furniture_tiles and see if any of 
        # the tiles match the query tile
        for furniture in self.furniture_tiles:
            furniture_tile_x, furniture_tile_y = self.coord2tileidx(furniture[0], furniture[1])
            if furniture_tile_x == m and furniture_tile_y == n:
                return True
        else:
            return False
    ...
    def is_position_furnished(self, pos):
        ...
        """
        # Convert the position XY to a tile index and use the is_tile_furnished
        # helper method
        tile_x, tile_y = self.coord2tileidx(pos.get_x(), pos.get_y())
    ...
    def is_position_valid(self, pos):
        ...
        """
        # Check to see if the position is furnished. If it is, then we know that
        # the position is in the room
        if self.is_position_furnished(pos):
            return False
        else:
            # Otherwise, check to see if the position is in the room using the 
            # EmptyRoom is_position_in_room helper method
            return self.is_position_in_room(pos)
    ...
    def get_num_tiles(self):
        ...
        """
        # Subtract the number of furnished tiles from the total number of tiles
    ...
    def get_random_position(self):
        ...
        """
        randpos = Position(self.width*random.random(), self.height*random.random())
        # Check that the position is valid, loop until we get a valid position
        while not self.is_position_valid(randpos):
            randpos = Position(self.width*random.random(), self.height*random.random())
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # Move robot to new random position and clean the tile
        ...
        if self.room.is_position_valid(newpos):
            ...
            # New position isn't valid, set direction and do nothing else
            self.direction = 360.0*random.random()  # Random returns 0.0 <= N < 1.0, multiply by 360 to scale
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # Test to see if we go faulty
        if self.gets_faulty():
            # We're faulty, don't clean and just update direction
            self.direction = 360.0*random.random()  # Random returns 0.0 <= N < 1.0, multiply by 360 to scale
        else:
            # Move robot to new random position and clean the tile
            newpos = self.position.get_new_position(self.direction, self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        bots = []
        for ii in range(num_robots):  # Add our robots
            bots.append(robot_type(room, speed, capacity))
        
        # Start the sim
        # For each timestep, iterate through the bots and run their update&clean
        # method
        # Iterate until the proportion of clean tiles is greater than or equal 
        # to the min_coverage threshold
        step = 0
        pcleantiles = room.get_num_cleaned_tiles() / room.get_num_tiles()
        while pcleantiles < min_coverage:
            step += 1
            for bot in bots:
        ...
        ntimesteps.append(step)

    return (sum(ntimesteps) / len(ntimesteps))
...
#       of a 20x20 room?
#
#    The StandardRobot cleans faster than the FaultyRobot in all scenarios. This
#    is not unexpected, as the FaultyRobot will, on average, be cleaning less
#    often for a given number of time steps than the StandardRobot. As the 
#    number of robots increases, the margin between the two robot types narrows,
#    as the number of FaultyRobot errors per time step is mitigated by the 
#    correctly functioning robots. 
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#
#    For the same number of robots, the FaultyRobot performs consistently worse
#    than the StandardRobot when cleaning rooms of various aspect ratios. As the
#    aspect ratio of the room increases, both robot types incur a penalty to 
#    time to clean. The FaultyRobot is affected more significantly than the 
#    StandardRobot. The linear relationship between time to clean and aspect 
#    ratio can be explained as a decrease in valid angles for the random 
#    movement, since the arc decreases as the height of the room decreases. I'm 
#    not sure why the trend is so much steeper for the FaultyRobot.
